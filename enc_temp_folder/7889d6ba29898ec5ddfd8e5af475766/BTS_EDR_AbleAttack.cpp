...
void UBTS_EDR_AbleAttack::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
    ...
    if (AbleAttack)
    {
        DrawDebugSphere(World, Center + (ForwardVector * (Range/2)), Range, 16, FColor::Blue, false, 0.2f);
    }
    else
    {
        DrawDebugSphere(World, Center + (ForwardVector * (Range / 2)), Range, 16, FColor::Red, false, 0.2f);
    }
